IF EXISTS (SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[gather_wait_stats_2008]') and OBJECTPROPERTY([object_id], N'IsProcedure') = 1)
	DROP PROCEDURE [dbo].[gather_wait_stats_2008] ;
go
CREATE PROCEDURE [dbo].[gather_wait_stats_2008] (@Clear INT = 0)
