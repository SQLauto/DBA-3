...
DECLARE @custompath NVARCHAR(500), @allow_xpcmdshell bit, @duration tinyint, @logdetail bit, @diskfrag bit, @ixfrag bit, @ixfragscanmode VARCHAR(8), @bpool_consumer bit, @gen_scripts bit, @dbScope VARCHAR(256)

/* Best Practices Check - PTO Clinic Edition - pedro.lopes@microsoft.com (http://toolbox/BPCheck; http://aka.ms/ezequiel)
...
v1.1 - 09-10-2013 - Fixed issue with false positives in Purity Check;
...
v1.2 - 25-10-2013 - Added blocking chains (over 5s);
...
v1.2.0.1 - 28-10-2013 - Fixed issues with logic to exclude MS shipped databases;
v1.2.1 - 03-11-2013 - Added owner info to Database Information subsection;
...
v1.2.2.1 - 11-11-2013 - Removed cursor from VLF checks;
...
v1.2.2.2 - 14-11-2013 - Added Dynamics GP database exclusions;
						Fixed syntax issue in System Configurations checks.
v1.2.3 - 19-12-2013 - Fixed presentation issue in Pending I/O Requests subsection;
						Fixed conversion issue in Statistics Update subsection.
v1.2.4 - 13-01-2014 - Fixed issue with database exclusions;
...
v1.2.4.1 - 06-02-2014- Fixed issue with Redundant index checks on servers with CS collations.
v1.2.5	- 21-02-2014 - Added LowMemoryThreshold and OOM information to Server Memory subsection;
...
v1.2.6 - 14-03-2014 - Duplicate script generation includes logic to account for PK when several IXs are unique;
...
v1.2.7 - 01-04-2014  - Fixed issue in Hypothetical objects drop script generation;
						Fixed issue in several subsections when offline DBs exist;
						Fixed issue introduced in v1.2.6 version where some checks would skip user DBs;
...
v1.2.7.1 - 03-04-2014 - Added Service Pack Supportability check;
						Fixed overflow issue in clock hand notifications;
						Fixed issue with clock hand notifications and SQL 2005.
v1.2.8 - 23-04-2014 - Changed Pagefile free space check;
...
v1.2.9 - 09-05-2014 - Detailed categorization of memory related waits;
...
v1.3 - 04-06-2014 - Refined search for duplicate and redundant indexes;
						Added partition misalign test (offset < 64KB);
						Added Buffer Pool Extension info subsection.
v1.3.1 - 09-06-2014 - Fixed issue with duplicate and redundant index script generation.
v1.3.2 - 10-06-2014 - Fixed calculation in page file checks;
						Added process paged out check (besides existing based in errorlog search).
v1.3.3 - 08-07-2014 - Fixed backup checks reporting wrong size;
						Changed backup chain based checks to ignore copy_only backups;
						Updated Service Pack Supportability check.
v1.3.4 - 16-07-2014 - Added Backups and Database files in same location check;
						Enhanced Hints Usage check with info from sql modules;
						Fixed Objects naming conventions checks not reporting issues.
v1.3.5 - 17-09-2014 - Changed Hypothetical object search scope to all DBs, including MS shipped;
						...
						Expanded TF checks.
v1.3.6.1 - 20-10-2014 - Fixed hypothetical statistics search;
...
v1.3.7 - 12-11-2014 - Fixed conversion issue with Buffer Pool Consumers section;
...
v1.3.8 - 10-12-2014 - Fixed issue with Database file autogrows last 72h subsection on case sensitive instances;
...
v1.3.8.1 - 16-12-2014 - Fixed issues in Enterprise features usage subsection on SQL 2012 and AlwaysOn in use.
v1.3.9 - 30-01-2015 - Fixed conversion issue in Plan_use_ratio check;
						Added HADR info to Database Information subsection;
						Added NUMA info to Server Memory subsection.
v1.3.9.1 - 06-02-2015 - Fixed NUMA info collection issue up to SQL 2008R2 introduced in v1.3.9.
v1.3.9.2 - 13-02-2015 - Fixed NUMA info collection issue on SQL 2012 and above introduced in v1.3.9;
						Added support to scope only specific databases by ID.
v1.3.9.3 - 15-03-2015 - Fixed some information in "Database Information" section not being collected, introduced in v1.3.9.2.
v1.3.9.5 - 11-04-2015 - Fixed System health error checks conversion error;
...
IF (SELECT SUM(CAST(size_in_bytes AS bigint))/1024/1024 AS Size_MB
	...
	WHERE cacheobjtype LIKE '%Plan%' AND usecounts > 1)
BEGIN
	SELECT 'Performance_checks' AS [Category], 'Plan_use_ratio' AS [Check], '[WARNING: Amount of single use plans in cache is high]' AS [Deviation], CASE WHEN @sqlmajorver > 9 AND @adhoc = 0 THEN '[Consider enabling the Optimize for ad hoc workloads setting on heavy OLTP ad-hoc worloads to conserve resources]' ELSE '' END AS [Comment]
END
ELSE
BEGIN
...
BEGIN
	SELECT 'Performance_checks' AS [Category], 'Inefficient_Plans_Reads' AS [Check], query_plan, 
		(SELECT REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
			...
			NCHAR(1),N'?'),NCHAR(2),N'?'),NCHAR(3),N'?'),NCHAR(4),N'?'),NCHAR(5),N'?'),NCHAR(6),N'?'),NCHAR(7),N'?'),NCHAR(8),N'?'),NCHAR(11),N'?'),NCHAR(12),N'?'),NCHAR(14),N'?'),NCHAR(15),N'?'),NCHAR(16),N'?'),NCHAR(17),N'?'),NCHAR(18),N'?'),NCHAR(19),N'?'),NCHAR(20),N'?'),NCHAR(21),N'?'),NCHAR(22),N'?'),NCHAR(23),N'?'),NCHAR(24),N'?'),NCHAR(25),N'?'),NCHAR(26),N'?'),NCHAR(27),N'?'),NCHAR(28),N'?'),NCHAR(29),N'?'),NCHAR(30),N'?'),NCHAR(31),N'?') 
			FOR XML PATH(''), TYPE) AS [statement],
		[execution_count], [total_worker_time]/[execution_count] AS [Avg_Worker_Time], [total_physical_reads]/[execution_count] AS [Avg_Physical_Reads],
...
OPTION (MAXDOP 2)'
	...
	BEGIN
		...
		BEGIN
			SELECT 'Index_and_Stats_checks' AS [Category], 'XTP_HashIX_Health_AvgChain_EmptyBuckets' AS [Check], '[WARNING: Some databases have high avg chain lenght and high empty buckets count. Verify if there are many rows with duplicate index key values or there is a skew in the key values]' AS [Deviation]
...
WHERE [type] = 'P' AND [Object] LIKE 'sp[_]%'
	AND [Object] NOT IN ('fn_diagramobjects');
