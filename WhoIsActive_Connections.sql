...
SELECT [start_time]
      ...
      --,[session_id]
      ,LEFT(cast(sql_text as varchar(max)), 255) sql_text
      ,LEFT(cast(sql_command as varchar(max)), 255) sql_command
      ,[login_name]
      --,[wait_info]
      --,[tran_log_writes]
      --,[CPU]
  ...
  WHERE collection_time BETWEEN '2019-06-18 07:55:00.000' AND '2019-09-25 13:00:00.000'
				AND database_name = 'SMS_WEL'
...
exec dbamon.dbo.sp_WhoIsActive @help=1