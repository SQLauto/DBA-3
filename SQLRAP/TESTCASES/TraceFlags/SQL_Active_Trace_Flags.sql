--Signature="C89F684225BF14E4" 

--/**************************************************************************************************************/
--/**************************************************************************************************************/
--/****    Microsoft SQL Server Risk Assesment Program                                                       ****/
--/****                                                                                                      ****/
--/****    Enumerate all active traceflags                                                                   ****/
--/****                                                                                                      ****/
--/****    Updated 2008.Mar.08 by wardp to more closely mimic 1.9 functionality                               ****/
--/****    Updated 2009.Mar.19 by wardp to address bug 326697                                                ****/
--/****    Added validation for SQL Server 2008 - djaiswal 9/21/2009  Ref CR 375891                                                                                                   ****/
--/****		Temp comment for Raj from Devin, the 10 script does not find session flags, only displays                                                                                                 ****/
--/****     global flags.                                                                                                 ****/
--/****                                                                                                      ****/
--/****                                                                                                      ****/
 ...
 if  9 = (select substring(@version, 1, 1))
     ...
     if  8 =  (select substring(@version, 1, 1))
         begin
			...
			IF @instancename ='default'
			   BEGIN
                SET @instanceregname = N'MSSQLSERVER'
				SELECT @StartupParametersRegstring='SOFTWARE\Microsoft\Microsoft SQL Server\' + @instanceregname + '\MSSQLServer\Parameters'
				SELECT @sysadmin_onlyRegstring=N'SOFTWARE\Microsoft\Microsoft SQL Server\' + @instanceregname + '\SQLServerAgent'
			   END
			ELSE
			   BEGIN
                SET @instanceregname = @instancename
				SELECT @StartupParametersRegstring='SOFTWARE\Microsoft\Microsoft SQL Server\' + @instanceregname + '\MSSQLServer\Parameters'
				SELECT @sysadmin_onlyRegstring=N'SOFTWARE\Microsoft\Microsoft SQL Server\' + @instanceregname + '\SQLServerAgent'
			   END
			...
			/*get trace flags enabled with dbcc*/
			insert into #tbl_TraceFlagSet (TraceFlag, TraceFlagStatus)	
				exec ('dbcc tracestatus(-1) with NO_INFOMSGS')
