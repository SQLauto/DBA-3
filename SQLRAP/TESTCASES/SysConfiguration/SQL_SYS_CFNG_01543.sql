--Signature="BC763F3B223CF248"

--/**************************************************************************************************************/
--/**************************************************************************************************************/
--/****    Microsoft SQL Server Risk Assesment Program                                                       ****/
--/****                                                                                                      ****/
--/****      Display configuration parameters different than default                                         ****/
--/****                                                                                                      ****/
--/****      10/06/08 - Ward Pond - CR 245579                                                                ****/
--/****      10/14/09 - djaiswal - CR 379851                                                                                                ****/
--/****                                                                                                      ****/
--/****                                                                                                      ****/
--/****                                                                                                      ****/
--/****                                                                                                      ****/
--/****                                                                                                      ****/
--/****    Copyright � Microsoft Corporation. All rights reserved.                                           ****/
--/**************************************************************************************************************/
--/**************************************************************************************************************/
declare @2008default int;
set     @2008default = 0;

declare @2005default int;
set     @2005default = 0;

declare @2000default int;
set     @2000default = 0;

declare @version char(12);
set     @version =  convert(char(12),serverproperty('productversion'));

 if  '10' = (select substring(@version, 1, 2))
     begin
         select  serverproperty('machinename') as 'servername',                                            -- 'Server Name'
                 isnull(serverproperty('instancename'),serverproperty('machinename')) as 'instancename',   -- 'Instance Name'
                 a.name,                                                                                   -- 'Configuration Name'
                 @2008default as 'default',                                                                -- 'Target/Default Value'
                 a.value,                                                                                  -- 'Set Value'
                 a.value_in_use as 'run_value'                                                             -- 'Run Value'
           from  master.sys.configurations a
          where  a.configuration_id = 1543
           and   a.value_in_use <> @2008default 

     end
 else 
 if  9 = (select substring(@version, 1, 1))
     begin
         select  serverproperty('machinename') as 'servername',                                            -- 'Server Name'
                 isnull(serverproperty('instancename'),serverproperty('machinename')) as 'instancename',   -- 'Instance Name'
                 a.name,                                                                                   -- 'Configuration Name'
                 @2005default as 'default',                                                                -- 'Target/Default Value'
                 a.value,                                                                                  -- 'Set Value'
                 a.value_in_use as 'run_value'                                                             -- 'Run Value'
           from  master.sys.configurations a
          where  a.configuration_id = 1543
           and   a.value_in_use <> @2005default 

     end
 else 
     if  8 =  (select substring(@version, 1, 1))
         begin
             select  serverproperty('machinename') as 'servername',                                            -- 'Server Name'
                     isnull(serverproperty('instancename'),serverproperty('machinename')) as 'instancename',   -- 'Instance Name'
                     c.name,                                                                                   -- 'Configuration Name' 
                     @2000default as'default',                                                                 -- 'Target/Default Value'
                     a.value,                                                                                  -- 'Set Value'
                     b.value as 'run_value'                                                                    -- 'Run Value'
               from  master..sysconfigures a 
               join  master..syscurconfigs b on b.config = a.config
               join  master..spt_values c on b.config = c.number
			   where  a.config = 1543 
                and  c.type = 'C'
                and  b.value <> @2000default
         end;