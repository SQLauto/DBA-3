-- Signature="EB9485616B990BDF"

--/**************************************************************************************************************/
--/**************************************************************************************************************/
--/****    Microsoft SQL Server Risk Assesment Program                                                       ****/
--/****                                                                                                      ****/
--/****    SQL Census Data Collector for SQL 2008.sql                                                        ****/
--/****    builds infrastructure for and executes SQL Census code mapping for a SQL Server 2008 instance     ****/
--/****                                                                                                      ****/
...
BEGIN
	...
	SET @ErrorString = 'Attempt to run SQL Server 2008 (SQL Server version 10) SQLRAP Data Collector on a SQL Server '
...
BEGIN
	DECLARE @ErrorString nvarchar(4000)
	SET @ErrorString = 'Attempt to run SQL Server 2008 SQLRAP Data Collector without first running Bootstrap process.  Process terminated.'
					  + CHAR(13) + CHAR(10)
...
BEGIN
	...
	RETURN
		CASE 
			WHEN @objectid = -1
				THEN N''--== Informational: Database ''
						+ DB_NAME(@databaseid)
						+ N'' excluded from SQL Census data collection and report, both by user directive ==--''
			WHEN @objectid = -2
				THEN N''--== Warning: Database ''
						+ DB_NAME(@databaseid)
						+ N'' EXCLUDED from test case execution but included in SQL Census data collection, both by user directive. ==--''
			WHEN @objectid = -3
				THEN N''--== ERROR: Database ''
						+ DB_NAME(@databaseid)
						+ N'' EXCLUDED from SQL Census data collection and then included in test case execution, both by user directive.  Re-run SQL Census Data Collector to resolve. ==--''
			ELSE NULL
		END
...
FROM master.sys.databases s (NOLOCK)
LEFT OUTER JOIN
